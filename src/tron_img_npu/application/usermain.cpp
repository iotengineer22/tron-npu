...
extern void print_bg_font_18(d2_device *handle, d2_point _xs, d2_point _ys, float scaling, char *_str);
extern void task_3(INT stacd, void *exinf);
}
...
LOCAL T_CTSK ctsk_3 = {
    ...
    .tskatr  = TA_HLNG | TA_RNG3,
    .task    = (FP)task_3,
    .itskpri = 11,                 // 描画タスク(10)よりやや低く設定して画面保護
    .stksz   = 32768,              // 推論・ポスト処理用に32KB確保
    .bufptr  = NULL
};

ID tskid_1;                          // Task ID number (global)
ID tskid_2;                          // Task ID number (global)
ID tskid_3;                          // Task ID number (global)
...
LOCAL void task_1(INT stacd, void *exinf)
{
    ...
    while (1)
    {
        ...
        if (p_camera_capture_buffer_stored != NULL)
        {
            ...
            // 画面全体のクリア (起動時にゼロクリアされており、余白は黒のまま維持されるため毎フレームのクリアは省略して帯域を節約)
            // d2_clear(d2_handle, 0xFF000000); // 黒クリア
            ...
            // D/AVE 2Dによる分類推論テキスト・パラメータのオーバーレイ描画
            char text_buf[64];
            
            // 1. モデル情報を左上に描画
            print_bg_font_18(d2_handle, 130, 20, 1.0f, (char*)"Model: MobileNet V1");
            
            // 2. 推論時間を右上に描画
            sprintf(text_buf, "Inference: %04d ms", (int)g_ai_inference_time_ms);
            print_bg_font_18(d2_handle, 680, 20, 1.0f, text_buf);
            
            // 3. Top-1 分類結果を画面下部に大きく描画
            if (g_ai_result_new)
            {
                const char** labels = getLabelPtr();
                uint16_t cat0 = g_ai_classification[0].category;
                float prob0 = g_ai_classification[0].prob;
                
                // ラベル名からカンマ以降を切り詰め、短く綺麗な形式に成形
                char clean_label[32] = {0};
                int j = 0;
                for (; labels[cat0][j] != '\0' && labels[cat0][j] != ',' && j < 24; j++) {
                    clean_label[j] = labels[cat0][j];
                }
                clean_label[j] = '\0';
                
                sprintf(text_buf, "Result: %s (%d%%)", clean_label, (int)(prob0 * 100.0f));
                print_bg_font_18(d2_handle, 130, 540, 1.2f, text_buf);
            }
            else
            {
                print_bg_font_18(d2_handle, 130, 540, 1.0f, (char*)"Result: Inference starting...");
            }
            ...
            // [推論トリガー] カメラフレームデータから 224x224 RGB888 へ変換し、NPUタスクをキック
