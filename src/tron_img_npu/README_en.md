...
# MobileNet Image Classification - NPU Version (tron_img_npu)

## Program Description
This program is an edge AI application running on μT-Kernel 3.0 that performs ultra-real-time image classification using the "MobileNet V1" deep learning model, accelerated by the "Arm Ethos-U55" NPU embedded in the Renesas RA8 microcontroller.
It displays the captured 320x240 RGB565 camera image scaled to 800x600 using bilinear filtering. Concurrently, it converts the camera frame to a 224x224 RGB888 format and runs high-speed classification inference using the Ethos-U55 NPU driver. The Top-1 category name, confidence level (%), and hardware-accelerated inference latency are overlaid on the LCD screen in real time.
...
* **Memory**: External SDRAM (allocated for Triple Buffer framebuffers, model tensors, and model binaries)
* **Control I/O**:
...
3. **tskid_3 (task_3)** (Priority: 11, Stack size: 32KB)
   * AI inference task. Assigned a priority of 11. Receives the data conversion completion notification (`tk_wup_tsk`) from `task_1` and executes the MobileNet V1 inference on the Ethos-U55 NPU hardware.

## Processing Flow
1. **Initialization**: Configures SDRAM, GLCDC pin assignments, MIPI camera, Dave2D, and opens the Arm Ethos-U55 NPU driver instance.
2. **Rendering & Inference Kick Loop (task_1)**:
   ...
   * Draws the model name `Model: MobileNet V1` on the upper-left and NPU inference latency (`g_ai_inference_time_ms`) on the upper-right.
   * Overlays the classified category name and confidence level (%) on the bottom of the screen.
...
3. **Inference Loop (task_3)**:
   ...
   * Runs model inference on the Ethos-U55 NPU hardware. NPU acceleration significantly reduces latency to a few milliseconds compared to CPU execution.
   * Stores top probability results and sets `g_ai_result_new`.

## Key Parameters & Definitions
* **Model**: MobileNet V1 (Input: 224x224x3, int8 quantized Vela model compiled for Ethos-U55)
* `model_buffer_int8`: Input tensor buffer (224x224 RGB888 size: 150,528 bytes)
* `g_ai_inference_time_ms`: Hardware-accelerated inference execution latency (ms).
