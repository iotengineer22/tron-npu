...
{
#if (TIMER_TYPE == 0)
    // Unlock DWT write access (necessary for Cortex-M7/M33/M55/M85)
    #define DWT_LAR_ADDR (DWT_BASE + 0xFB0)
    *((volatile uint32_t *)DWT_LAR_ADDR) = 0xC5ACCE55;
...
uint32_t TimeCounter_CountValueConvertToMs(uint32_t t1, uint32_t t2)
{
    ...
    return (uint32_t)(((uint64_t)(t2 - t1) * 1000) / R_FSP_SystemClockHzGet(FSP_PRIV_CLOCK_CPUCLK));
#else
    return (uint32_t)(((uint64_t)(t2 - t1) * 1000) / R_FSP_SystemClockHzGet(FSP_PRIV_CLOCK_ICLK));
#endif
#elif (TIMER_TYPE == 1)
    return (t2 - t1) / 10;
#endif
