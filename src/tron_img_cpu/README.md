...
# MobileNet画像分類 CPU版 (tron_img_cpu)

## プログラム概要
このプログラムは、μT-Kernel 3.0 上で動作し、MIPI-CSI2 カメラ (OV5640) からの画像に対して、ディープラーニングモデル「MobileNet V1」を用いてリアルタイムで画像分類 (Image Classification) を実行するエッジ AI プログラムです。
カメラから取得した 320x240 RGB565 画像を 800x600 に拡大表示するとともに、入力画像を 224x224 RGB888 へ変換し、TensorFlow Lite Micro を用いて CPU 上で分類推論を実行します。推論結果の Top-1 カテゴリ名、確信度 (%)、および推論処理時間を液晶画面上にリアルタイムにオーバーレイ表示します。
...
* **メモリ**: 外部 SDRAM (トリプルバッファフレームバッファ領域、テンソルバッファ領域)
* **制御 I/O**:
...
3. **tskid_3 (task_3)** (優先度: 11, スタックサイズ: 32KB)
   * AI推論タスク。優先度 11。`task_1` から入力データ変換完了通知 (`tk_wup_tsk`) を受け取り、TFLite Micro を用いて CPU 上で MobileNet V1 推論を実行、分類確率を算出して結果を更新します。

## 処理フロー
1. **初期化処理**: SDRAM 初期化、GLCDC ピン構成、MIPI カメラリセットと初期化、Dave2D の初期化。
2. **メイン描画・推論起動ループ (task_1)**:
   * Vblank 同期スリープから復帰。
   * Dave2D でカメラ画像 (320x240) を 800x600 に中央拡大コピー。
   * 画面左上に `Model: MobileNet V1` を描画。
   * 画面右上に前回の推論時間 (`g_ai_inference_time_ms`) を描画。
   * 新しい推論結果が得られている場合、分類結果ラベルと確信度 (%) を画面下部に大きく描画。
   * キャッシュのクリーンを行い、液晶バッファの切り替えを GLCDC に要求。
   * カメラ画像データを 224x224 RGB888 に変換し `model_buffer_int8` にコピー、キャッシュクリア後に `tk_wup_tsk(tskid_3)` で推論タスクを起動。
3. **推論処理ループ (task_3)**:
   * 起床待ち。
   * TFLite Micro を用いて MobileNet V1 の推論を実行。
   * 分類されたカテゴリ確率の上位結果をグローバル配列に格納し、フラグ `g_ai_result_new` を更新。

## 主要なパラメータ・定義
* **モデル**: MobileNet V1 (入力: 224x224x3, int8 量子化)
* `model_buffer_int8`: 推論入力バッファ (224x224 RGB888 用: 150,528 バイト)
* `g_ai_inference_time_ms`: 推論時間 (ms)
