...
LOCAL void task_1(INT stacd, void *exinf)
{
    ...
    while (1)
    {
        ...
        if (p_camera_capture_buffer_stored != NULL)
        {
            ...
            // カメラ画像が上書きしない左端と右端の余白エリア（X:0〜111、X:912〜1023、Y:0〜599）を黒でクリアする
            d2_setcolor(d2_handle, 0, 0xFF000000); // 黒色
            d2_renderbox(d2_handle, (d2_point)(0 << 4), (d2_point)(0 << 4), (d2_width)(112 << 4), (d2_width)(600 << 4));
            d2_renderbox(d2_handle, (d2_point)(912 << 4), (d2_point)(0 << 4), (d2_width)(112 << 4), (d2_width)(600 << 4));
            ...
            for (int i = 0; i < AI_MAX_DETECTION_NUM; i++)
            {
                ...
                {
                    // 96x96 座標系から LCD 画面上の 800x600 エリア（X=112オフセット）への変換 (スケール値 = 6.25f)
                    float fx = (float)g_ai_detection[i].m_x * 6.25f + 212.0f;
                    float fy = (float)g_ai_detection[i].m_y * 6.25f;
                    float fw = (float)g_ai_detection[i].m_w * 6.25f;
                    float fh = (float)g_ai_detection[i].m_h * 6.25f;
                    ...
                    // クラス名と確信度の描画 (例: Pico: 95%)
                    char val_str[32];
                    if (g_ai_detection[i].m_class >= 1 && g_ai_detection[i].m_class <= 4)
                    {
            ...
            // AIトータル処理時間（検出時間）と検出された各PCBの数の描画
            int fpc_count = 0, nrf_count = 0, pico_count = 0, xiao_count = 0;
            for (int i = 0; i < AI_MAX_DETECTION_NUM; i++)
            {
                if (g_ai_detection[i].m_w > 0 && g_ai_detection[i].m_h > 0)
                {
                    if (g_ai_detection[i].m_class == 1) fpc_count++;
                    else if (g_ai_detection[i].m_class == 2) nrf_count++;
                    else if (g_ai_detection[i].m_class == 3) pico_count++;
                    else if (g_ai_detection[i].m_class == 4) xiao_count++;
                }
            }
            ...
            char count_str1[64];
            sprintf(count_str1, "FPC: %d | nRF: %d", fpc_count, nrf_count);
            
            char count_str2[64];
            sprintf(count_str2, "Pico: %d | Xiao: %d", pico_count, xiao_count);

            d2_setcolor(d2_handle, 0, 0xFF00FF00); // 明るい緑色
            print_bg_font_18(d2_handle, (d2_point)120, (d2_point)10, 1.0f, time_str);
            print_bg_font_18(d2_handle, (d2_point)120, (d2_point)35, 1.0f, count_str1);
            print_bg_font_18(d2_handle, (d2_point)120, (d2_point)60, 1.0f, count_str2);
