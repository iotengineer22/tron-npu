...
uint8_t * p_camera_capture_buffer_stored = NULL;
...
fsp_err_t camera_init (bool use_test_mode)
{
    ...
    tm_printf((UB *)"[Camera Init] Product ID High: 0x%02X, Low: 0x%02X\n", reg_val1, reg_val2);

    if (!((reg_val2 == CORRECT_PRODUCT_ID_L_REV1A) || (reg_val2 == CORRECT_PRODUCT_ID_L_REV2A) || (reg_val2 == CORRECT_PRODUCT_ID_L_REV2C)))
    {
        tm_printf((UB *)"[Camera Init] ERROR: Product ID Mismatch! Device lock error.\n");
...
#if CAMERA_CONFIG_WRITE_VERIFY
            if (array->reg_num == 0x3008)
            {
                // Skip readback verification for software reset / power down registers
            }
            else
            {
                ...
                {
                    CAMERA_I2C_DEBUG ("Write ADDR:[0x%04x] Mismatch! Written:[0x%02x] ReadBack:[0x%02x]\r\n",array->reg_num,array->value,read_value);
...
static fsp_err_t ov5640_configure_clocks()
{
    ...
    rdSensorReg16_8(0x3035, &read_val);
    if (read_val != reg_3035_value)
    {
        tm_printf((UB *)"[Clock Config] Register 0x3035 Mismatch! Written: 0x%02X, Read: 0x%02X\n", reg_3035_value, read_val);
    ...
    rdSensorReg16_8(0x3036, &read_val);
    if (read_val != reg_3036_value)
    {
        tm_printf((UB *)"[Clock Config] Register 0x3036 Mismatch! Written: 0x%02X, Read: 0x%02X\n", reg_3036_value, read_val);
        err = FSP_ERR_INVALID_DATA;
    }
    ...
    rdSensorReg16_8(0x3037, &read_val);
    if (read_val != reg_3037_value)
    {
        tm_printf((UB *)"[Clock Config] Register 0x3037 Mismatch! Written: 0x%02X, Read: 0x%02X\n", reg_3037_value, read_val);
        err = FSP_ERR_INVALID_DATA;
    }
    ...
    rdSensorReg16_8(0x3108, &read_val);
    if (read_val != reg_3108_value)
    {
        tm_printf((UB *)"[Clock Config] Register 0x3108 Mismatch! Written: 0x%02X, Read: 0x%02X\n", reg_3108_value, read_val);
        err = FSP_ERR_INVALID_DATA;
    }
...
void cam_vin_callback(capture_callback_args_t *p_args)
{
    ...
    interrupt_status.mask = p_args->interrupt_status;
    static int first_frame_logged = 0;

    // 本物のカメラエラーイベント (VIN_EVENT_ERROR = 2) のみを検知して出力
    if (event == VIN_EVENT_ERROR)
    {
        static int error_logged_count = 0;
        if (error_logged_count < 10)
        {
            tm_printf((UB *)"[VIN CB] TRUE ERROR Event: %d, Status Mask: 0x%08X\n", (int)event, (unsigned int)interrupt_status.mask);
            error_logged_count++;
        }
    }
    ...
    if(event == VIN_EVENT_NOTIFY)
    {
        if (interrupt_status.bits.frame_complete)
        {
            ...
            {
                tm_printf((UB *)"[VIN CB] First frame captured successfully!\n");
