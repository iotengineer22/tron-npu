...
vision_ai_app_err_t image_rgb565_to_int8 (const void * p_input_image_buff, void * p_output_image_buff, uint16_t in_width, uint16_t in_height, uint16_t out_width, uint16_t out_height)
{
    ...
    for (uint32_t y = 0; y < out_height; y++)
    {
        for (uint32_t x = 0; x < out_width; x++)
        {
            ...
            uint8_t r_5 = (uint8_t)((input >> 11) & 0x1F);
            uint8_t g_6 = (uint8_t)((input >> 5) & 0x3F);
            uint8_t b_5 = (uint8_t)(input & 0x1F);

            // Scale to 8-bit [0, 255]
            uint8_t r_8 = (uint8_t)((r_5 * 255) / 31);
            uint8_t g_8 = (uint8_t)((g_6 * 255) / 63);
            uint8_t b_8 = (uint8_t)((b_5 * 255) / 31);

            // Convert to signed INT8 range [-128, 127] and write to output (RGB channel order)
            *p_output++ = (int8_t)(r_8 - 128);
            *p_output++ = (int8_t)(g_8 - 128);
            *p_output++ = (int8_t)(b_8 - 128);
        }
    }
