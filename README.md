...
本システムでは、μT-Kernel 3.0 の優先度ベース of マルチタスクスケジューリングと、独立した専用アクセラレータ（NPU/GPU）を組み合わせることで、**「UI/カメラ表示の応答性（60Hz）の完全維持」** と **「推論のバックグラウンド実行（数ms〜数十ms）」** の両立を可能とし、極めて実用的で低遅延なリアルタイムAIエッジアプリケーションを実証しました。
...
### 3-1. μT-Kernel 3.0 マルチタスク設計（並列駆動）
...
### 3-2. Dave2D GPU と Ethos-U55 NPU の協調動作シーケンス
...
### 3-3. CPUからNPUへのAI推論高速化（リアルタイム性能と決定論的制御）
...
* **実機測定によるNPU高速化ベンチマーク効果 (CPU実行 vs NPU実行)**:
  * **MobileNet V1 画像分類**: CPU上で約1.5秒（1,512ms）かかっていた推論を **約 17 ms（約88.9倍の高速化）** に推論。
...
* **技術概要**:
  ...
  ![μT-Kernel 3.0 マルチタスク並行シリアル出力 概念図](img/diagram_serial_test.jpg)
* **コードにおける重要ポイント**:
...
* **技術概要**:
  ...
  ![I2Cカメラ接続通信シーケンス 概念図](img/diagram_i2c_test.jpg)
* **コードにおける重要ポイント**:
...
* **技術概要**:
  ...
  ![液晶 GLCDC トリプルバッファリング 概念図](img/diagram_d2_test.jpg)
* **コードにおける重要ポイント**:
...
* **技術概要**:
  ...
  ![MIPI-CSI2 カメラ・液晶表示データフロー 概念図](img/diagram_mipi_test.jpg)
* **コードにおける重要ポイント**:
...
- **技術概要**:
  ...
  ![MobileNet V1 画像分類 NPU推論パイプライン 概念図](img/diagram_img_npu.jpg)
- **コードにおける重要ポイント**:
...
- **技術概要**:
  ...
  ![YOLO顔検出 NPU/GPU非同期マルチタスクフロー 概念図](img/diagram_yolo_npu.jpg)
- **コードにおける重要ポイント**:
...
- **技術概要**:
  ...
  ![FOMOグリッドセルベース物体検出 概念図](img/diagram_fomo_npu.jpg)
- **コードにおける重要ポイント**:
