...
開発環境（e2 studio）でソースコードを変更して再ビルドした後は、本リポジトリの**ルートディレクトリ**にある **`collect_binaries.bat`** をダブルクリックして実行するだけで、各プロジェクトから最新の `.elf` と `.srec` を自動で本 `debug/` フォルダへ再集約・クリーンアップ（RFPエラー対策 of OFSフィルタ処理含む）することができます。
