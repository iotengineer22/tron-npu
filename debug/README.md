...
* **`cpu_ai_versions/`**:
  NPU高速化の効果を比較するために開発された、**CPU通常処理版**のAIプログラム of SRECです（推論速度の違いを測定するために使用します）。
* **`elf_files/ (参考)`**:
...
> USBケーブルを挿し込んだ際、稀に下図のように液晶画面が真っ白になってフリーズする現象が発生することがあります。この現象が発生した場合は、**数秒〜数十秒ほど時間を置いてから、USBケーブルを一度抜いて挿し直してください。**
>
...
1. プロジェクト画面が開いたら、中央の「プログラムファイルとユーザー鍵」枠にある **[ファイルの追加と削除(A)...]** または入力欄 of 右側のボタンをクリックします。
